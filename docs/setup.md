...
1. Copy `.env.example` to `.env` and fill required values:
   ...
   - Optional demo bypass (non-production only): `DEMO_MODE=true`, `DEMO_BYPASS_ENABLED=true`, and `DEMO_USER_EMAIL`
2. Install dependencies: `npm install`.
3. Generate Prisma client: `npx prisma generate`.
4. Run migrations: `npm run prisma:migrate`.
5. Seed baseline metadata (production-safe, no demo data): `npm run prisma:seed`.
6. Optional demo workspace seed for guided validation: `npm run prisma:seed:demo`.
...
- Set `DEMO_BYPASS_ENABLED=true` to explicitly allow demo bypass in local/dev environments.
