...
2. Interactive customization using plain chat prompts for policy and org input
...
Ask in chat with a multi-choice list and allow selecting multiple policies:

Example for governance policies:
- "Which policies would you like to generate?" (multi-select)
- Options: Present top 4 most relevant policies based on their needs, user can select multiple

**Repeat** selection prompts for different categories if generating multiple policies:
...
Ask in chat about output preferences:

**Question Set 1 - Output Formats:**

- "Which output formats do you need?" (multi-select)
- Options:
...
Collect organization-specific information via plain chat prompts and concise option lists.
...
2. "What industry does your organization operate in?" (options: Technology, Finance, Healthcare, Government, Manufacturing, Retail)
3. "How many employees does your organization have?" (options: <50, 50-500, 500-1000, 1000+)

**Question Set 2 - Governance:**

Ask:
1. "Who is the executive responsible for these policies?" (options: CISO, CTO, CRO, VP InfoSec, IT Director)
2. "Which department owns these policies?" (options: InfoSec, IT, Risk, Compliance)
3. "What is the contact email for policy questions?" (free text)

**Question Set 3 - Lifecycle:**

Ask:
1. "When should these policies take effect?" (provide common options like "Next month", "Start of quarter", "Custom date")
2. "How often will these policies be reviewed?" (options: Quarterly, Semi-annually, Annually, Bi-annually)

**Question Set 4 - Compliance:**

Ask (multi-select where applicable):
1. "Which compliance frameworks must you meet?" (options: ISO 27001, SOC 2, NIST CSF, CIS Controls v8, GDPR, HIPAA, PCI-DSS)
2. "Are there specific regulatory requirements?" (options: None, GDPR, HIPAA, PCI-DSS, SOX, GLBA, FERPA)
...
Based on user's format selections from Phase 2, convert each markdown output using repository scripts or available local tooling.

**If Word (.docx) selected:**

Convert markdown to Word and apply:
...
Convert markdown to HTML and apply:
...
Convert markdown to PDF and apply:
...
**IMPORTANT:**
- Prefer repo-native scripts first (`scripts/generate_docx_html_pdf.py`).
- If a required converter is not available in the local environment, report what is missing and continue with available formats.
...
4. Collects company info using chat prompts
...
4. Uses chat prompts for customization
